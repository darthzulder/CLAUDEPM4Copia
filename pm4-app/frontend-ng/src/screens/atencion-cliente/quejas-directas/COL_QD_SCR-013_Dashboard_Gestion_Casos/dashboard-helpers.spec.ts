...
// Lógica pura que sostiene TODO el dashboard: la pantalla se apoya en estas funciones, así que
// cubrirlas acá es lo que hace que el spec de pantalla pueda concentrarse en la UI.
// `mapRequestToCaso` recibe el set de feriados por argumento, así que es testeable sin red
// (misma decisión de diseño que core/business-days.ts).
//
// Portado de `frontend/src/.../dashboardHelpers.test.ts` sin cambios de semántica; se agrega
// abajo el caso del bug heredado del Área, que en React no tenía cobertura.
...
describe('mapRequestToCaso', () => {
  ...
  it('⚠ bug heredado — areaResponsable sale de qd_strResponsableRole (un ROL), no del código de área', () => {
    // Comportamiento ACTUAL de React, portado a propósito. `types.ts` documenta este campo como
    // «CÓDIGO de la colección QD_COLLECTIONS.area» y el filtro Área compara contra ese código,
    // pero acá llega un nombre de rol ⇒ el filtro no matchea nunca (ver el caso del spec de
    // pantalla). El campo que cerraría el círculo es `qd_strAssigneeArea`.
    //
    // Este `it` NO valida un comportamiento deseado: fija el actual para que el día que negocio
    // decida cuál de los dos campos manda, el test se ponga rojo y señale qué cambiar.
    const objCaso = mapRequestToCaso(
      {
        id: 1,
        status: 'ACTIVE',
        data: { qd_strResponsableRole: 'Analista SAC', qd_strAssigneeArea: '35' },
      },
      SET_VACIO,
    );
    expect(objCaso.areaResponsable).toBe('Analista SAC');
    expect(objCaso.areaResponsable).not.toBe('35'); // el código de área se ignora hoy
  });
});
