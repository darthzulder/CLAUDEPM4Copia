...
//
// Portado sin cambios de `frontend/src/core/fechaHora.ts`: es lógica pura de Date, sin
// nada de React ni de Angular. El único ajuste es el nombre del archivo (kebab-case, que
// es la convención del CLI de Angular).
//
// La guarda de zona horaria de la que dependen estos sellos vive en `zona-horaria.spec.ts`,
// no en el spec de este módulo: acá la TZ se fija en el script npm con `cross-env` y no en
// una config de test, así que es un requisito del workspace y no de este archivo.
