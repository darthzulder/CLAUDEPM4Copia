...
import { resolveFileId } from './request-files.types';

/**
 * Portado de `frontend/src/core/useRequestFiles.test.ts` con **paridad 1:1 de casos** (15: 8 bloques
 * `it` más las 7 entradas del `it.each`, que Vitest cuenta como un caso cada una) y sin cambios de
 * aserción; solo cambia la ruta del import.
 *
 * Solo se testea resolveFileId: es la parte pura del módulo (el hook useRequestFiles pega a
 * PM4 y pasa a ser `RequestFilesService` en la Fase 3b, con su propio spec). PM4 devuelve el
 * file_id de un campo de salida en formatos inconsistentes según el nodo del proceso, y esta
 * función es la que normaliza todos — de ahí las ramas raras.
 */
