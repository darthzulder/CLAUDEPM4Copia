...
| SEC-028 | SCR-009 | S1 Datos Precargados M1 (solo lectura) | Sección de formulario | 1 | 2 | No (no se renderiza) | No se renderiza; los datos M1 viajan en el payload (reset). | — |
...
| SEC-048 | SCR-0051 | S2 Clasificación Regulatoria (precargada M1) | Sección de formulario | 2 | 2 | Sí | Siempre | Clasificación SFC precargada desde el Momento 1; Producto, Momento, Servicio, Placa y Motivo son re-editables en M3 (cascada cat_matriz_motivos). Canal, Instancia, Admisión y Ente de Control siguen en solo lectura. |
...
| SEC-051 | SCR-0051 | S5 Asignación de Responsable | Sección de formulario | 5 | 2 | Sí | Siempre visible (solo lectura); editable al pulsar "Reasignar Queja". | Asignación del área y usuario responsable; siempre visible en solo lectura, se habilita para reasignar con el botón "Reasignar Queja". |
...
| SEC-062 | SCR-013 | S1 KPIs de SLA | Sección de tablero | 1 | 4 | Sí | Siempre | Tarjetas KPI de SLA (abiertos, próximos a vencer, vencidos, cerrados) sobre la lista completa. |
| SEC-063 | SCR-013 | S2 Filtros | Sección de formulario | 2 | 4 | Sí | Siempre | Filtros (tipo, estado, área, búsqueda) cliente-side con botones Aplicar/Limpiar. |
| SEC-064 | SCR-013 | S3 Tabla de Casos | Sección de tabla | 3 | 1 | Sí | Siempre | Tabla paginada de casos (8 por página) con semáforo de días y estado. |
| SEC-065 | SCR-013 | S4 Modal de Detalle | Modal | 4 | 2 | Condicional | Al pulsar 'Ver' en un caso | Detalle de solo lectura del caso con un único botón Cerrar. |
